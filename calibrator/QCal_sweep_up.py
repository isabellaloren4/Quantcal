...
se esse conjunto é held-out, a própria tabela, ou só o split atual:
  ...
  - eval_mode='in_sample': todos os subgrupos vão para a tabela e os erros são
    medidos nela mesma (tabela cumulativa inteira). É barato, mas err_reg fica
    otimista porque phi é avaliada onde foi treinada.
  - eval_mode='current': todos os subgrupos vão para a tabela (phi continua
    CUMULATIVA, como no in_sample), mas os erros são medidos SÓ nos subgrupos
    do split atual — n_prevalences batches (p.ex. 50). Conjunto de avaliação de
    tamanho FIXO e barato; err_reg fica otimista (subestimado), pois esses
    subgrupos também entraram no treino de phi. Use este modo quando quiser
    err_quant e err_reg avaliados sempre nos mesmos ~50 batches de X_val,
    aceitando o viés otimista do regressor.
...
-------------------------------------------------------------------
  ...
  critério (n_stop_cross_) costuma disparar no min_n_validation. Nesses modos
  o best_n_validation_ pode colapsar para 1 na maioria dos datasets — perdendo
  o poder de distinguir um dataset do outro. Só o 'held_out' mantém o err_reg
  out-of-sample e, portanto, uma seleção comparável entre datasets.
...
class QCalNValSweep:
    ...
    n_prevalences : int, padrão=50
        ...
        Em eval_mode='current' este é também o TAMANHO do conjunto de avaliação
        (os erros são medidos nesses n_prevalences batches do split atual).
    batch_size : int, padrão=100
        Tamanho de cada subgrupo UPP (repassado ao protocolo).
    eval_mode : {'held_out', 'in_sample', 'current'}, padrão='held_out'
        ...
        tabela cumulativa inteira (mais barato, porém otimista); 'current' mede
        só nos subgrupos do split atual (n_prevalences batches, tamanho fixo),
        também otimista/subestimado porque esses batches treinaram phi.
    ...
    def __init__(self, *, method_name='cc', clf=None,
        ...
        if eval_mode not in ('held_out', 'in_sample', 'current'):
            raise ValueError(
                f"eval_mode={eval_mode!r} inválido; "
                "use 'held_out', 'in_sample' ou 'current'"
            )
    ...
    def fit(self, X_train, y_train, name_data=None):
        ...
        for i in range(self.max_n_validation):
            ...
            linhas.append({
                ...
                'eval_mode': self.eval_mode,          # held_out, in_sample ou current
